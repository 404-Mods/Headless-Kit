...
const SlideUpText = forwardRef<SlideUpTextRef, SlideUpTextProps>(
  (
    ...
    const text = typeof children === "string"
      ? children
      : children?.toString() || "";
    // Seeded from the props so an auto-started animation is already "on" during the
    // first render — no effect round-trip, no flash of the hidden state.
    const shouldAutoStart = autoStart && !inView;
    const [isAnimating, setIsAnimating] = useState(shouldAutoStart);
    ...
    // Re-arm if the props flip after mount. Adjusting state during render is the
    // supported pattern here; doing it in an effect would cascade an extra render.
    const [wasAutoStart, setWasAutoStart] = useState(shouldAutoStart);
    if (shouldAutoStart !== wasAutoStart) {
      setWasAutoStart(shouldAutoStart);
      if (shouldAutoStart) setIsAnimating(true);
    }

    // Notify the consumer for auto-started runs. No state write, so no cascade.
    useEffect(() => {
      if (shouldAutoStart) onStart?.();
    }, [shouldAutoStart, onStart]);
